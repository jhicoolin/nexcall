...
function Header({ onCallDemo }: { onCallDemo: () => void }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-[#172033]/8 bg-[#f8f3eb]/95 backdrop-blur-xl">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        <a href="#top" className="flex items-center gap-3" aria-label="NexCall home">
          ...
          <span className="text-lg font-black tracking-wide text-[#172033] sm:text-xl">NEXCALL</span>
        </a>
        <div className="hidden items-center gap-7 text-xs font-black uppercase tracking-[0.16em] text-[#4b5a67] md:flex">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="inline-flex min-h-11 items-center transition hover:text-[#172033]">{item.label}</a>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button" onClick={onCallDemo} data-fallback-href="/?demo=1"
            className="system-button-primary hidden min-h-11 items-center gap-2 rounded-xl px-4 py-2 text-sm font-black transition hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-[#6f8f34]/25 sm:inline-flex"
          >
            <Phone size={17} aria-hidden="true" /> Call Demo
          </button>
          <button
            type="button" onClick={() => setMobileOpen((c) => !c)}
            className="system-button-primary flex h-11 w-11 items-center justify-center rounded-xl transition focus:outline-none focus:ring-4 focus:ring-[#6f8f34]/25 md:hidden"
      ...
      {mobileOpen && (
        <div className="mx-auto max-w-7xl px-4 pb-4 sm:px-6 md:hidden">
          <div className="rounded-2xl border border-[#172033]/8 bg-white p-3 shadow-lg shadow-[#172033]/8">
            {navItems.map((item) => (
              <a key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                className="block rounded-xl px-4 py-3 text-sm font-bold text-[#4b5a67] transition hover:bg-[#6f8f34]/8 hover:text-[#172033]">
                {item.label}
              </a>
            ))}
...
function CinematicHero({ onCallDemo }: { onCallDemo: () => void }) {
  return (
    <section id="top" className="relative flex min-h-screen flex-col justify-center overflow-hidden pt-20 sm:pt-24">
      ...
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 60% 50% at 85% 10%, rgba(111,143,52,0.07), transparent)" }} />
        <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 45% 45% at -5% 70%, rgba(111,143,52,0.05), transparent)" }} />
      </div>

      <div className="relative mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        {/* Badge */}
        <p className="mb-7 inline-flex items-center gap-2 text-[0.7rem] font-black uppercase tracking-[0.22em] text-[#6f8f34]">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#6f8f34]" aria-hidden="true" />
        ...
        <div className="hero-stagger-1 grid gap-12 lg:grid-cols-2 lg:items-start lg:gap-16">
          {/* LEFT */}
          <div>
            <p className="max-w-lg text-xl leading-[1.7] text-[#4b5a67]">
              <span className="font-semibold text-[#172033]">NexCall</span> helps local businesses answer when the team is busy, capture appointment requests, and keep follow-up organized.
            </p>
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <a href="#lead"
                className="inline-flex min-h-[56px] w-full items-center justify-center gap-2 rounded-[8px] bg-[#6f8f34] px-8 text-sm font-black tracking-wide text-white transition hover:scale-[1.02] hover:bg-[#5e7a2c] focus:outline-none focus:ring-4 focus:ring-[#6f8f34]/30 sm:w-auto">
                Request Setup <ArrowRight size={16} aria-hidden="true" />
              </a>
              <a href="#how-it-works"
                className="inline-flex min-h-[56px] w-full items-center justify-center gap-2 rounded-[8px] border border-[#172033]/12 px-8 text-sm font-semibold text-[#172033] transition hover:border-[#172033]/25 hover:bg-[#172033]/[0.04] sm:w-auto">
                See How It Works <ArrowRight size={15} aria-hidden="true" />
              </a>
            </div>
            <p className="mt-4 text-xs text-[#6b7280]">No card required. Setup takes about 60 seconds.</p>

            {/* Stat strip */}
            <div className="mt-10 grid grid-cols-2 gap-5 sm:grid-cols-4 lg:grid-cols-2">
              {[
                { value: "Answered", label: "Call coverage" },
                { value: "Live", label: "Front desk" },
                { value: "Fast handoff", label: "Team summary" },
                { value: "After hours", label: "Overflow ready" }
              ].map((s) => (
                <div key={s.label} className="border-l-2 border-[#6f8f34]/30 pl-4">
            ...
            <p className="mt-8 text-sm text-[#4b5a67]">Built for clinics, salons, contractors, and appointment-based local teams.</p>
          </div>

          {/* RIGHT — warm 3-stage call flow card */}
          <FrontDeskFlowCard />
        </div>
      </div>
    </section>
...
/* Warm 3-stage call flow card — replaces dashboard-style visual */
function FrontDeskFlowCard() {
  return (
    <div className="relative space-y-2">
      {/* Step 1: Call comes in */}
      <div className="rounded-2xl border border-[#172033]/8 bg-white p-5 shadow-sm shadow-[#172033]/5">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#6f8f34]/20 bg-[#6f8f34]/8">
            <Phone size={18} className="text-[#6f8f34]" aria-hidden="true" />
          </div>
          <div className="flex-1">
            <p className="text-[0.58rem] font-black uppercase tracking-[0.15em] text-[#6b7280]">Step 1</p>
            <p className="text-sm font-black text-[#172033]">Customer reaches out</p>
          </div>
        </div>
        <p className="mt-2 text-xs leading-5 text-[#4b5a67]">NexCall answers before the caller hits voicemail or a dead end.</p>
      </div>

      {/* Connector */}
      <div className="px-5">
        <div className="ml-4 h-6 w-px bg-gradient-to-b from-[#6f8f34]/30 to-[#6f8f34]/10" />
      </div>

      {/* Step 2: Details captured */}
      <div className="rounded-2xl border border-[#6f8f34]/20 bg-[#6f8f34]/[0.04] p-5">
        <div className="mb-3 flex items-center gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#6f8f34]/30 bg-[#6f8f34]/10">
            <ClipboardList size={18} className="text-[#6f8f34]" aria-hidden="true" />
          </div>
          <div className="flex-1">
            <p className="text-[0.58rem] font-black uppercase tracking-[0.15em] text-[#6f8f34]">Step 2</p>
            <p className="text-sm font-black text-[#172033]">NexCall captures the details</p>
          </div>
        </div>
        <div className="space-y-1.5">
          {(["Name + contact info", "Reason for calling", "Urgency level"] as const).map((item) => (
            <div key={item} className="flex items-center gap-2.5">
              <Check size={13} className="shrink-0 text-[#6f8f34]" aria-hidden="true" />
              <span className="text-xs font-bold text-[#4b5a67]">{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Step 3: Team brief */}
      <div className="rounded-2xl border border-[#172033]/8 bg-white p-5 shadow-sm shadow-[#172033]/5">
        <p className="mb-2 text-[0.58rem] font-black uppercase tracking-[0.15em] text-[#6b7280]">Step 3 · Your team receives</p>
        <div className="space-y-1.5 text-xs">
          <p className="text-[#4b5a67]"><span className="font-bold text-[#172033]">Caller:</span> Contact details on file</p>
          <p className="text-[#4b5a67]"><span className="font-bold text-[#172033]">Need:</span> Appointment this Thursday</p>
          <p className="text-[#4b5a67]"><span className="font-bold text-[#172033]">Next step:</span> Confirm available slot</p>
        </div>
        <div className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-[#6f8f34]/20 bg-[#6f8f34]/8 px-3 py-1">
          <span className="h-1.5 w-1.5 rounded-full bg-[#6f8f34]" aria-hidden="true" />
          <span className="text-[0.58rem] font-black text-[#6f8f34]">Your team follows up</span>
        </div>
      </div>
    </div>
  );
