...
import { deleteTask } from "@/lib/misato/runtime/service";
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as { id?: string; taskId?: string };
  const result = deleteTask(String(body.taskId || body.id || ""));
